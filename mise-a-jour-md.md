# 🍷 Dionysos — État du projet (à jour au 3 septembre 2026)
...
- **Base** : Google Sheets « Vino 3.0 » — onglets Vino · Historique · config/CONFIG · Suggestions
...
- **`scripts-scanner-v2.js`** : fourre-tout — scan, menu d'action, saisie manuelle, vin inconnu, Arrivée, Déplacer, Boire, Donner, Cave, À ranger, Sans cépage, Suggestions, Liste d'achat, Emplacements, Historique, Promotions, Recherche, facture, navigation, panneaux de filtres.
...
- `getInventoryData` renvoie TOUS les champs (vins actifs et vins à 0 bouteille) pour que la fiche se bâtisse en mémoire.
- `ALL_HISTORIQUE` en chargement paresseux : chargé à la première ouverture, invalidé après Boire et par RAFRAÎCHIR.
- `ficheDepuisMemoireV2(cb)` → `{wine, bottles}` (secours `getWineBottles`). Fournit les clés sans accent `Designation`/`Temperature`/`Cepage`.
...
## 🧭 Navigation
- **Burger** : ACCUEIL, CAVE, EMPLACEMENTS, HISTORIQUE, LISTE D'ACHAT, À RANGER, SANS CÉPAGE, PROMOTIONS SAQ, RAFRAÎCHIR. Chaque cible fait `cacherToutesPagesV2()` puis ouvre.
- **Accueil** : `#topNavV2`, 4 boutons en `space-evenly` — scan · SAQ (`ouvrirSAQV2` : app SAQ sur iPhone, saq.com ailleurs) · loupe Recherche · burger.
...
**Après création** (`enchainerMenuApresCreationV2`) : `checkWineExists`, puis resync `ALL_DATA`, puis menu. Si le vin n'est pas retrouvé → message d'erreur, PAS de menu vide.
...
**Blocs** : Information · Description + Prix · Dégustation · Production · Notes · Historique des plats (section masquée si vide) · Inventaire (lecture seule) · Photo (clic → `#photoV2Overlay`) · roundel ACTION puis roundel OÙ LE TROUVER · vérification silencieuse du prix (`verifierPrixV2`).
...
**Retour** (`fermerFicheV2`) : `menuScan` + origine → liste d'origine ; sinon menu d'action ; 'cave'/'achat'/'histo'/'promo'/'recherche'/'emplacements'/'aranger'/'sanscepage'/'suggestions' → leur page.
...
- Boutons : Vins en double · Cépages doubles (meuble choisi) · **Cépages manquants** — avec meuble : présents ailleurs, absents ici ; sans meuble : cépages de ma liste absents du stock actif, avec les vins à racheter dessous.

**Historique** — `ALL_HISTORIQUE` paresseux, filtres Mets · Vin · Accord. Corps par vin : carte vin → fiche, cartes mets → éditeur Corriger (gabarit commun, plat en textarea). Origine retrouvée par Code SAQ d'abord, code-barres ensuite, jamais par le nom. **Le roundel « Ajouter » est dans le panneau de filtres**, pas sous les cartes.

**Suggestions** — liste des notes de sommeliers par vin. Filtres sommelier / couleur / en cave. Ajout depuis la fiche (section Suggérer) ou par scan (`ajouterDepuisSuggestionsV2` → drapeau `suggestionsV2Attente` → scanner). Écran d'ajout/correction : nom du vin, sommelier (menu + « Ajouter un sommelier »), note.
...
**Recherche** — accès par la loupe de l'accueil. Un champ qui fouille tous les champs de tous les vins (accents et casse ignorés, minimum 2 caractères). Exclus : row, bottle, Statut, Meuble/Rangee/Espace, dates, Source, Photo URL. Résultats groupés par vin.

**Facture (reçu SAQ)** — photo d'une facture → `extraireRecuSAQ` (OCR Drive) → liste de codes SAQ → validation un par un.

**Accords** — page dédiée + ajout d'un accord depuis le menu de la fiche : « + Ajouter » → champ texte, doublon coché au lieu d'être réécrit, sinon `ajouterAccordConfig` écrit dans la première cellule vide de la colonne Accords de config. L'autre téléphone le voit au prochain démarrage (RAFRAÎCHIR ne recharge pas CONFIG — accepté).
...
**Panneaux de filtres** — les 5 panneaux sont des coquilles vides remplies au chargement par `construirePanneauxV2()` depuis la table `PANNEAUX_V2`.
...
**Lecture** : `getConfig` · `getInventoryData` · `getWineBottles` · `checkWineExists` · `checkLocationAvailable` · `getHistorique` · `getSuggestions` · `getAccords` · `getSuccursales` · `getToutesSuccursales` · `getSuccursalesDisponibles` · `getCodeBarresFromCodeSAQ`
...
- **Sections jamais rédigées à partir du code** : suggestions/sommeliers, facture, page Accords — décrites ci-dessus de mémoire de séance, à vérifier.
...
- **3 septembre 2026** — Bouton SAQ de l'accueil (`ouvrirSAQV2`). **Panne du scan** : tout vin neuf tombait sur un menu « Vin inconnu » sans rien écrire ; cause = déploiement figé sur une vieille version, aucun code en faute. Corrections : resync mémoire après création, refus d'ouvrir un menu vide, vrais messages d'erreur transmis, `suggestionsV2Attente` éteint au scan depuis l'accueil, comparaison du Code SAQ par `memeCodeBarre`, cache de 5 min sur la lecture SAQ (la page était téléchargée deux fois), `User-Agent` ajouté, code mort de l'onglet « Vins scannés » retiré.
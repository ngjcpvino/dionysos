...
## 🧭 Navigation
- **Burger** (`burgerV2Click`) : ACCUEIL · CAVE · À RANGER · SANS CÉPAGE · SUGGESTIONS · LISTE D'ACHAT (cible `racheter`) · EMPLACEMENTS · HISTORIQUE · PROMOTIONS SAQ · FACTURE · ACCORDS · RAFRAÎCHIR. Chaque cible fait `cacherToutesPagesV2()` puis ouvre sa page ; ACCUEIL cache tout et vide `menuActionV2Context` ; RAFRAÎCHIR resynchronise `ALL_DATA` et vide `ALL_HISTORIQUE`. Une cible inconnue répond « À venir ».
...
**Accords mets-vins** (`ouvrirAccordsV2`) — page qui répond à « j'ai tel ingrédient, quel vin ouvrir ? ». Deux mécaniques à ne pas confondre avec les **catégories d'accords** d'un vin (champ Accords de la fiche, colonne config).
- **Source** : onglet Sheet « Accords » (Cépage · Aliment · Nuance · Source), lu une fois par `getAccords` → `ALL_ACCORDS`, puis mémoire.

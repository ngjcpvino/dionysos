...
- **En-tête du mode rond** (4 septembre 2026, `#photoV2-entete`) : au-dessus du cercle, **toujours visible** — nom, origine (pays · région · appellation), cépage. Le nom au centre du cercle reste le secours quand la photo manque ou casse. Caché en mode fiche.
- **Au gabarit des pages VIN** (4 septembre 2026) : le nom est un `h2.titre-1`, l'origine et le cépage un seul `.texte-secondaire` séparés par un `<br>` — mêmes classes que le menu d'action, aucun style recopié. Les classes `.photo-entete-nom` et `.photo-entete-ligne` sont supprimées.
- **Empilage plutôt que superposition** (4 septembre 2026) : l'en-tête n'est plus en `position:absolute` par-dessus le cercle — il occupe une bande **de hauteur fixe (110 px)** en haut du contenu, qui passe en `flex-start` + `padding-top: 60px` comme `.modale-haut`. Le cercle prend la bande restante (`aspect-ratio: 1`, `max-height: 100%`, `margin: auto 0`) : ⚠️ hauteur fixe OBLIGATOIRE, sinon le cercle changerait de taille à chaque glissement selon la longueur du nom. Le ✕ perd sa position à 2 h et revient au `.btn-fermer` standard, en haut à droite.

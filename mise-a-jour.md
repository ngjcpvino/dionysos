...
- **Panneau de filtre** (ordre voulu, dans `PANNEAUX_V2.achat` → `avant` + `filtres`) : (1) interrupteur **« Que les vins en promotion »** (`achatV2-promo` / `toggleAchatPromoSeulV2`, même patron que « en cave ») ; (2) titre **SUCCURSALE** + le menu succursale ; (3) titre **AFFICHER** + les 5 listes ; (4) titre **FILTRER** (ajouté d'office par `construirePanneauxV2`) + Couleurs · Pays · Cépage · **Appellation** (nouveau) · Pastille. Plus de roundel « Ne pas racheter ».
...
- **Cartes** — zone de droite EN PILE (une seule règle CSS scopée `#achatV2-cartes .carte-droite { flex-direction: column; align-items: flex-end }`) : **rond à cocher panier** en haut (inchangé, `.coche-panier`, garde `carte-vide`), puis le **prix** (`.achat-prix`) — et si promo : prix barré · prix promo · image points bonis (`.achat-bonis`, `blocPrixAchatV2` / `boisPointsAchatV2`) —, puis la **dispo** si une succursale est choisie. Un vin que j'ai ouvre sa fiche (provenance `'achat'`) ; une Découverte ouvre saq.com ; le rond reste partout (clé `cleCartePanierV2`, repli Code SAQ/Nom).
- **Tri** : `decouvertes` par cépage (sections `emp-meuble`), les autres par pays.
- **Succursale** (`filtresAchatV2.succ`) : une succursale → quantité (`verifierDispoSAQ_GRAPHQL_V1`) ; **« Mes favorites »** (`FAV`) → vérifie toutes les favorites d'un coup (`getSuccursalesDisponibles`) ; **« Toutes les succursales »** (`TOUTES`) → toucher une carte cherche les **3 succursales proches** en stock + **itinéraire Apple Plans** (`dispoProchesAchatV2`, `maps.apple.com`). « Gérer mes favorites » via `gererFavoritesV2('achatV2')` (seul appelant restant).

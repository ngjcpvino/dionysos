...
- **AFFICHER = 5 modes** (`achatV2Mode`), ordre à l'écran : `achat` (à racheter : Racheter=Oui & 0 en stock, ou Panier=Oui) · `favoris` (Favori = Oui, l'étoile ★) · `suggestions` (« Liste propositions », Racheter vide) · `decouvertes` (promos SAQ sur des vins que je n'ai pas) · `nepasracheter` (Racheter=Non). Base construite dans `baseAchatV2()`.
  - **Sous « Liste propositions » : choix du sommelier** (11 sept. 2026). Menu déroulant `#achatV2-sommelier-choix` (champ `achatV2-f-sommelier-display` + menu `achatV2-f-sommelier-menu`, indenté), **visible seulement en mode `suggestions`** (`remplirFiltresAchatV2` bascule son `display`). État `filtresAchatV2.sommelier`, liste bâtie depuis `ALL_SUGGESTIONS`. `baseAchatV2` (branche suggestions) ne garde que les propositions dont le vin a une proposition de ce sommelier (`sommeliersDuVinV2`). Remis à '' au changement de mode et au reset ; compte dans l'entonnoir doré.

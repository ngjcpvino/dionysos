...
- **Bouton « Accords SAQ » dans l'app (codé 7 sept. 2026 — PAS encore publié ni redéployé)** : `majAccordsSAQ()` (nouvelle route `doPost`) enchaîne `majFamillesAccordsVins(200)` — relancé par le front tant que `restants>0` (`termine:false`) — puis `majRecettesSAQ()`, et renvoie `{ termine, faits, familles, ajoutees }`. Front : `majAccordsSaqBoucleV2` (Outils, avant « Curieux Bégin », même icône `.item-icone`) relance auto, puis resynchronise `ALL_DATA` (`getInventoryData`) et vide `ALL_RECETTES` (familles écrites sur les vins), et affiche « ✓ Accords SAQ à jour (X recettes) » ou la **vraie** erreur. **Demande le redéploiement de `Code.gs`.**
